...
async function initDB() {
  ...
  try {
    ...
    // Remove duplicate emails first, then add unique constraint
    try {
      // Delete duplicates keeping the most recent one
      await pool.query(`
        DELETE FROM contacts a USING contacts b
        WHERE a.id < b.id AND a.email = b.email AND a.email != ''
      `);
      // Delete contacts with no email
      ...
      console.log('Email constraint already exists or skipped:', constraintErr.message);
    }
...
app.post('/api/apollo-search', async (req, res) => {
  try {
    ...
    console.log('Apollo search found:', people.length, 'contacts');
    
    // Step 2: Enrich each contact to get email (costs 1 credit each)
    const enriched = [];
    for (const person of people) {
      if (person.email) {
        // Already has email from search
      ...
      try {
        ...
        // Skip if we don't have enough info to enrich
        if (!firstName && !linkedinUrl) {
          console.log('Skipping - no name or linkedin');
          continue;
        }

        const enrichPayload = { reveal_personal_emails: true };
        if (firstName) enrichPayload.first_name = firstName;
        if (lastName && lastName !== 'undefined') enrichPayload.last_name = lastName;
        if (orgName) enrichPayload.organization_name = orgName;
        if (linkedinUrl) enrichPayload.linkedin_url = linkedinUrl;
        if (person.id) enrichPayload.id = person.id;
        
        console.log('Enriching:', firstName, lastName !== 'undefined' ? lastName : '', '|', orgName, '| linkedin:', !!linkedinUrl);
        ...
        if (enrichData.person?.email) {
          enriched.push({ ...person, email: enrichData.person.email, phone_numbers: enrichData.person.phone_numbers || [] });
          console.log('Got email:', enrichData.person.email);
        } else {
          console.log('No email returned. Status:', enrichData.person?.email_status, 'Error:', enrichData.error || 'none');
        }
      } catch(err) {
    ...
    console.log('Apollo final:', enriched.length, 'with emails out of', people.length, 'total');
    res.json({ ...data, people: enriched });
  } catch (error) {
