...
async function initDB() {
  ...
  try {
    ...
    try {
      ...
      const chk = await pool.query(`SELECT 1 FROM pg_constraint WHERE conname = 'contacts_email_unique'`);
    ...
    } catch(constraintErr) { console.log('DB setup note:', constraintErr.message); }
...
app.post('/api/scan-sources', async (req, res) => {
  try {
    ...
    // Parse and return clean items array so frontend doesn't need extractText
    let items = [];
    try {
      const texts = (data.content || []).filter(b => b.type === 'text').map(b => b.text).join('\n');
      const raw = texts.replace(/```json|```/g, '').trim();
      const s = raw.indexOf('['), e = raw.lastIndexOf(']');
      if (s !== -1 && e !== -1) items = JSON.parse(raw.substring(s, e + 1));
    } catch(pe) { console.log('Parse error in scan-sources:', pe.message); }

    console.log(`scan-sources: scanned ${batch.map(b=>b.name).join(', ')} — ${items.length} items`);
    res.json({ items, sourcesScanned: batch.map(s => s.name) });
...
app.post('/api/monitor-sites', async (req, res) => {
  try {
    ...
    const body = {
      ...
      messages: [{
        role: 'user',
        content: `Search for recent news and announcements from these healthcare companies that signal buying opportunities for ROI/Release of Information, denial recovery, or medical records exchange:\n\n${siteList}\n\nReturn ONLY a valid JSON array:\n[{"type":"news","org":"company name","headline":"recent news or announcement","signal":"specific sales opportunity this creates","urgency":"high|medium|low","contact":"best decision-maker title","tags":["relevant","tags"],"source":"site name","receivedAt":"${new Date().toLocaleTimeString()}"}]\nOnly return JSON array.`
      }]
    };
    ...
    // Parse and return clean items array
    let items = [];
    try {
      const texts = (data.content || []).filter(b => b.type === 'text').map(b => b.text).join('\n');
      const raw = texts.replace(/```json|```/g, '').trim();
      const s = raw.indexOf('['), e = raw.lastIndexOf(']');
      if (s !== -1 && e !== -1) items = JSON.parse(raw.substring(s, e + 1));
    } catch(pe) { console.log('Parse error in monitor-sites:', pe.message); }

    console.log(`monitor-sites: checked ${targetSites.map(s=>s.name).join(', ')} — ${items.length} items`);
    res.json({ items, sitesMonitored: targetSites.map(s => s.name) });
  } catch (error) { res.status(500).json({ error: error.message }); }
});
