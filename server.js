...
app.post('/api/apollo-search', async (req, res) => {
  try {
    ...
    // Step 1: Search for contacts (no emails returned here by design)
    const searchPayload = {
      page: 1,
      per_page: 25,
      person_titles: [title],
    ...
    // Step 2: Enrich each contact using waterfall enrichment to get emails
    const enriched = [];
    for (const person of people) {
      try {
        const enrichPayload = {
          ...
          run_waterfall_email: true,  // Apollo's correct way to get emails
          reveal_personal_emails: true,
        };
        ...
        if (email) {
          enriched.push({ ...person, email, phone_numbers: enrichData.person?.phone_numbers || [] });
          console.log('Got email:', person.first_name, person.last_name, '|', email);
        } else {
          console.log('No email:', person.first_name, person.last_name, '| status:', enrichData.person?.email_status || 'unknown');
        }
      } catch(err) {
    ...
    console.log('Apollo final:', enriched.length, 'with emails out of', people.length);
